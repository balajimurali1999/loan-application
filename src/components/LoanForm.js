...
export default function LoanForm() {
    const userData = { name: 'balaji', dateOfBirth: '2023-05-22', gender: 'male', panId: 'ABXXX5XXXXC', email: 'balaji@gmail.com', branch: 'bangalore' }
    const formikForm = useFormik({
        initialValues: {
            name: userData.name ? userData.name : '',
            dateOfBirth: userData.dateOfBirth ? userData.dateOfBirth : '',
            gender: userData.gender ? userData.gender : '',
            panId: userData.panId ? userData.panId : '',
        ...
        validationSchema: Yup.object({
            name: Yup.string().required('Enter Application name'),
            email: Yup.string(),
            branch: Yup.string(),
                ...
                <div className='row mb-3 align-items-start'>
                    ...
                    {basicDetailFields.map((ele, ind) => (
                        <div className='col ' key={ind}>
                            ...
                            <input name={ele.name} onBlur={formikForm.handleBlur} disabled={userData[ele.name] ? true : false} defaultValue={formikForm.values[ele.name]} type={ele.type} className="form-control" id={ele.name} />
                        </div>
                    ))}
                </div>
